...
public class Blue_Far extends LinearOpMode {
    ...
    private void updateStateMachine() {
        switch (state) {
            ...
            // State 18: Blocker reset delay, then slow-drive forward into intake 2 end
            case 18:
                if (actionTimer.getElapsedTimeSeconds() >= TRANSFER_RESET_DELAY) {
                    activeTargetRPM = TARGET_RPM_FUTURE;
                    middleTransfer.setPower(1.0);
            ...
            // State 19: Wait to reach intake 2 end, then slow-drive back to pre-intake 2
            case 19:
                if (!follower.isBusy()) {
                    // Stay at slow speed for the back-out
                    follower.followPath(toPreIntake2Back, true);
            ...
            // State 20: Wait to return to pre-intake 2, then restore full speed and head to shoot
            case 20:
                if (!follower.isBusy()) {
                    follower.setMaxPower(1.0);
                    follower.followPath(toShootFromIntake2, true);
            ...
            // State 21: Arrive at shoot pose — stop transfer
            case 21:
                if (!follower.isBusy()) {
                    middleTransfer.setPower(0);
                    setState(22);
            ...
            // State 23: Shoot duration, close gate, then park
            case 23:
                if (actionTimer.getElapsedTimeSeconds() >= SHOOT_TIME_2) {
                    //  Gate.setPosition(GATE_CLOSED);
                    shooter.setHoodAnglePos(0.5);
            ...
            // State 24: Wait to park, then done
            case 24:
                if (!follower.isBusy()) {
                    setState(-1);
                }
