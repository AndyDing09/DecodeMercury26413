...
public class autofarredShooter extends LinearOpMode {
    ...
    private final Pose startPose   = new Pose(  48, -60, Math.toRadians( 90)); // <-- CHANGE THIS
    private final Pose shootPose   = new Pose(  48, -40, Math.toRadians( 90)); // <-- CHANGE THIS (aimed at far target)
    private final Pose pickupPose1 = new Pose(  24, -20, Math.toRadians(180)); // <-- CHANGE THIS (first pickup)
    ...
    private void shootBurst(int count) {
        for (int i = 0; i < count && opModeIsActive(); i++) {
            ...
            // blocker open -> push -> retract -> blocker restore
            transferBlocker.setPosition(blockerClosed); // you used 0.65 during transfer
